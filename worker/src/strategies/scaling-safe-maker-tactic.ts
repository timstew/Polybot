/**
 * Scaling Safe Maker Tactic — safe-maker with adaptive bid sizing.
 *
 * Starts small (5/side), ramps up after paired windows, ramps down after
 * one-sided losses. Prevents catastrophic loss from large one-sided fills.
 */
...
const DEFAULTS: ScalingParams = {
  bid_offset: 0.02,
  max_pair_cost: 0.93,
  min_bid_size: 5,
  default_bid_size: 5, // start small
  conviction_bias: 2.0,
...
class ScalingSafeMakerTactic implements WindowTactic {
  readonly id = "scaling-safe-maker";
  readonly displayName = "Scaling Safe Maker";
  readonly description = "Safe maker with adaptive bid sizing — starts small, ramps up on paired wins, down on losses";
  ...
  onEnter(tc: TacticContext): void {
    const p = { ...DEFAULTS, ...tc.params } as ScalingParams;
    const { signal } = tc;
    ...
    // currentBaseSize persists across windows via orchestrator's shared tactic state
    const currentBaseSize = (tc.params.currentBaseSize as number) ?? p.min_bid_size;
    tc.window.tacticState = {
      bidSize: currentBaseSize,
  ...
  async onTick(tc: TacticContext): Promise<void> {
    ...
    const effBidSize = Math.max(3, Math.round(ts.bidSize * Math.min(1.0, (wDurMs / 60_000) / 15)));
    if (w.upBidOrderId) {
  ...
  private async checkFills(tc: TacticContext, p: ScalingParams): Promise<void> {
    const { ctx, window: w } = tc;
    for (const side of ["UP", "DOWN"] as const) {
      ...
      if (status.status === "MATCHED") {
        ...
        ctx.log(`FILL ${side} [scaling-safe]: ${w.market.title.slice(0, 25)} ${status.size_matched}@${status.price.toFixed(3)} inv=${w.upInventory}/${w.downInventory}`);
        ...
        await ctx.db.prepare(
          ...
          `orch-scsafe${side.toLowerCase()}-${crypto.randomUUID()}`, ctx.config.id,
          tokenId, w.market.slug, `${w.market.title} [ORCH SCSAFE ${side}]`,
  ...
  private async updateQuotes(tc: TacticContext, p: ScalingParams): Promise<void> {
    ...
    const effectiveBaseSize = Math.max(3, Math.round(ts.bidSize * durationScale));
  ...
  private async sellExcess(tc: TacticContext, p: ScalingParams, label: string): Promise<void> {
    const { ctx, window: w } = tc;
    ...
    for (const side of ["UP", "DOWN"] as const) {
      ...
      w.sellCount++;
      ctx.log(`${label} ${side} [scaling-safe]: ${excess}@${sellPrice.toFixed(3)} pnl=$${pnl.toFixed(2)}`);
      await ctx.db.prepare(
        `INSERT OR IGNORE INTO strategy_trades (id, strategy_id, token_id, market, title, side, price, size, fee_amount, timestamp, pnl)
         VALUES (?, ?, ?, ?, ?, 'SELL', ?, ?, 0, datetime('now'), ?)`
      ).bind(
        `orch-scsafesell-${crypto.randomUUID()}`, ctx.config.id, tokenId, w.market.slug,
...
registerTactic("scaling-safe-maker", () => new ScalingSafeMakerTactic());
